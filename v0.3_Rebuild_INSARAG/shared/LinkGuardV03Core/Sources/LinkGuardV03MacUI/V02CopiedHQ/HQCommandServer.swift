...
class HQCommandServer: ObservableObject {
    ...
    @MainActor private func handleWiFiMessage(_ msg: WiFiMessage, connID: String) {
        ...
        case let rawType where USARMessageType(rawValue: rawType) != nil:
            guard let payloadData = msg.payload.data(using: .utf8),
                  ...
                  wirePayload.messageType == rawType else { return }
            onUSARMessage?(wirePayload)
            if let relayData = encodeWiFiMessage(msgType: rawType, payload: wirePayload) {
    ...
    func sendUSARMessage<Payload: Codable>(_ envelope: USARProtocolEnvelope<Payload>, targetDeviceIDs: [String]? = nil) {
        ...
        sendToDevices(data, targetDeviceIDs: targetDeviceIDs, fallbackToBroadcastOnNoMatch: true, context: envelope.messageType.rawValue)
    }
        ...
        queue.async { [weak self] in
            ...
            if sentCount == 0, fallbackToBroadcastOnNoMatch {
                ...
                for conn in self.connections {
                    ...
                    conn.send(content: data, completion: .contentProcessed { error in
                        if let error { print("[HQ-Server] Fallback send failed: \(error)") }
                    })
                }
            }
        }
