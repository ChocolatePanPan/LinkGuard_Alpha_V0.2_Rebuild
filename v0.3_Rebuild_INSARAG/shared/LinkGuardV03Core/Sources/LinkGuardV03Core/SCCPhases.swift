...
public enum SCCPhaseCatalog {
    public static let phases: [SCCPhase] = [
        ...
        SCCPhase(
            ...
            capability: "指派搜索任務",
            purpose: "戰術指揮",
            requiredFeatures: [.taskAssignment, .commandDispatch, .quickCommand],
            requiredPermissions: [.issueCommand, .updateTask],
            relatedMessageTypes: [.taskUpsert, .commandUpsert],
        ...
        SCCPhase(
            ...
            capability: "傷患總覽",
            purpose: "醫療協調",
            requiredFeatures: [.patientLocation, .hospitalCapacityView],
            requiredPermissions: [.viewMedicalSummary, .manageEvacuation],
            relatedMessageTypes: [.patientUpsert, .evacuationRequestUpsert, .hospitalCapacityUpsert],
            primarySections: [.operations, .logistics],
            implementationState: .coreBacked
        ),
        SCCPhase(
            ...
            capability: "檢傷分類統計",
            purpose: "MCI管理",
            requiredFeatures: [.startTriage, .disasterStatistics],
