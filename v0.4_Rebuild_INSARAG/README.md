# LinkGuard v0.4 Rebuild INSARAG

本資料夾建立 v0.4 的資料夾骨架、README、各版本獨立 Xcode project，以及共享 Swift framework。Mac、iOS、iPad 角色 project 均有獨立 target/scheme；共用程式暫沿用 `shared/LinkGuardV03Core`，讓各版本引用同一套模型、權限、同步協議與 field UI。

v0.4 的方向是 USAR/INSARAG + ICS 架構，採用共享核心與角色分流版本。Mac HQ 不再是唯一操作中心，UCC 作為最高協調與資料權威，SCC 作為現場指揮中心，iOS 保留原角色 app，並新增單一 `LinkGuard-iPhone` 現場入口。

## Current status

v0.4 目前是 shared core + app target + role field shell + Mac copied HQ UI 的產品化前階段，不是完整 INSARAG 現場系統。已完成的是核心資料模型、同步骨架、角色權限/功能矩陣、可 build 的 field app targets，以及 iOS/iPad FieldUI 的本地 Phase 2 閉環；本版新增 `LinkGuard-iPhone`，開啟後可選 TL/TE/EMT/VO 多個現場身份，並依選擇載入對應 appID、權限矩陣與功能頁籤。主要缺口仍是真實 GPS/相機/語音/NFC/地圖能力、正式同步 server/provisioning、Mac v0.4 指揮 UI 與 field drill 驗證。
...
- `LinkGuard-iPhone`：v0.4 單一 iPhone 現場入口，啟動後選 TL/TE/EMT/VO 身份。
...
- `shared/LinkGuardV03Core/`：v0.4 暫沿用的 Swift package framework。
...
- USAR team capability profile：shared core 已加入正式「城市搜索與救援隊隊伍概況表」A/B/C/D 資料模型、`teamCapabilityReportUpsert` sync message、FieldUI A/B/C/D 填寫表單、snapshot 儲存、audit event、Mac UCC/SCC summary state，以及 active copied-HQ 的接收與顯示頁。
- Unified iPhone launch：`LinkGuard-iPhone` 啟動後先選擇 TL-01、TL-02、TE-01、EMT-01、EMT-02、VO-01 或 VO-02，再重建對應 `FieldAppController`。

驗證指令：

```bash
cd v0.4_Rebuild_INSARAG/shared/LinkGuardV03Core
swift test
```

## Versioning

目前版本基準是 `0.4.0-alpha.1`，由 shared framework 的 `LinkGuardVersionInfo.current` 與 `LinkGuard-iPhone` target build number 對齊；若正式發版，再建立 Git tag `v0.4.0-alpha.1`。

每次實作更新後都必須更新版本號。v0.4 alpha 線目前先以本資料夾內 `Versioning.swift` 和 Xcode target build settings 為準；若要接回 repo-level release 流程，再同步更新根目錄 `VERSION` / `VERSION.json` / `scripts/version.sh`。
