...
struct TeamCapabilityReport: Codable, Identifiable, Equatable {
    ...
    var personnelSummary: String {
        "\(L("出隊")) \(totalMembers) · \(L("搜救犬")) \(searchDogCount ?? 0)"
    }

    var vehicleSummary: String {
        let weight = equipmentWeightTons ?? 0
        let volume = equipmentVolumeCubicMeters ?? 0
        return String(format: "%@ %.1ft · %@ %.1fm³", L("裝備重量"), weight, L("體積"), volume)
    }
    ...
    var capabilitySummary: String {
        var items: [String] = []
        if hasTechnicalSearch == true { items.append(L("技術搜索")) }
        if hasDogSearch == true { items.append(L("犬搜索")) }
        if hasRescueCapability == true { items.append(L("營救")) }
        if hasMedicalCapability == true { items.append(L("醫療")) }
        if hasHazmatDetection == true { items.append(L("危險品偵檢")) }
        if (structuralEngineerCount ?? 0) > 0 { items.append(L("結構工程師 %lld", structuralEngineerCount ?? 0)) }
        if canEstablishOSOCCRDC == true { items.append(L("OSOCC/RDC")) }
        if canSupportUSARCoordination == true { items.append(L("USAR 協調支援")) }
        ...
        return items.isEmpty ? L("未填寫能力項目") : items.joined(separator: "、")
    }
